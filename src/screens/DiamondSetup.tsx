...
export default function DiamondSetup({ onBack, onStart, onNavigate }: Props) {
  const setupGame = useLudoStore(state => state.setupGame);
  const { equippedGotiId, profile, earnedDiamonds = 0 } = useUserStore();
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-6 flex flex-col items-center max-w-lg mx-auto w-full">
      <div className="w-full flex items-center justify-between mb-8">
        ...
        {onNavigate && (
           <button onClick={() => { audio.playClick(); onNavigate('upiRedeem'); }} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-cyan-400 font-bold text-xs uppercase tracking-wider rounded-xl border border-cyan-900 transition-colors">
              VIEW REDEEM
           </button>
        )}
      </div>

      <div className="bg-slate-800/60 border border-emerald-500/30 rounded-3xl p-8 text-center mb-8 shadow-xl w-full">
        <span className="text-5xl mx-auto mb-4 drop-shadow-[0_0_15px_rgba(16,185,129,0.5)] block">💰</span>
        <h1 className="text-3xl font-black text-white mb-2 tracking-wide uppercase">Earning Ludo</h1>
        ...
        <div className="bg-slate-900/50 rounded-xl p-4 text-center border border-slate-700 mb-8">
          <p className="text-slate-400 font-bold uppercase text-xs mb-1">Match Reward</p>
        ...
        <button
          onClick={handleStart}
          className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-black text-lg py-4 rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 uppercase tracking-wide"
        >
